...
const friend = {
       ...
       blockFriend: async (req: Request, res: Response, next: NextFunction) => {
              ...
              try {
                     const isFriend = await Friend.findOne({ friend: friendId, owner: userId })
                     if (!isFriend) { return next(new customError("You are not friends with this person", 400)) }
                     if (isFriend.blocked == true) {
                            return next(new customError("You already blocked this person", 400))
                     }
                     const blockFriend = await Friend.findByIdAndUpdate(isFriend._id, { $set: { blocked: true } },
       ...
       unblockFriend: async (req: Request, res: Response, next: NextFunction) => {
              ...
              try {
                     const isFriend = await Friend.findOne({ friend: friendId, owner: userId })
                     if (!isFriend) { return next(new customError("Request not found", 404)) }
                     if (isFriend.blocked == false) {
                            return next(new customError("You are still friends with this person", 400))
                     }
                     const blockFriend = await Friend.findByIdAndUpdate(isFriend._id, { $set: { blocked: false } },
                            { new: true, runValidators: true })
